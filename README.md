...
<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256" viewBox="0 0 256 256">
  ...
  <path d="M80 80v96l64-48z" fill="#3498db"/>
  <circle cx="128" cy="128" r="20" fill="#ffffff"/>
  <text x="128" y="200" font-family="Arial" font-size="16" font-weight="bold" fill="#ffffff" text-anchor="middle">影视库</text>
</svg>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <meta name="description" content="影视库PWA应用 - 提供丰富的影视资源">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="apple-mobile-web-app-status-bar-style" content="default">
    <meta name="apple-mobile-web-app-title" content="影视库">
    <link rel="apple-touch-icon" href="https://li.nstar.fun:8443/favicon.ico">
    <link rel="icon" href="https://li.nstar.fun:8443/favicon.ico" type="image/x-icon">
    <link rel="mask-icon" href="/icon.svg" color="#4CAF50">
    <style>
        body {
...
<body>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => {
                window.location.href = 'https://li.nstar.fun:8443';
            }, 1000);
        });
    </script>
</body>
...
// 配置session以保存cookie和认证信息
app.whenReady().then(() => {
  // 确保session启用了cookie存储
  session.defaultSession.webPreferences = {
    webSecurity: false,
    allowRunningInsecureContent: true,
    partition: 'persist:main', // 使用持久化会话
    storageQuota: 50 * 1024 * 1024 // 50MB存储配额
  };
  
  // 监听cookie变化，用于调试
  session.defaultSession.cookies.on('changed', (event, cookie, cause, removed) => {
    console.log('Cookie变化:', cookie, '原因:', cause, '已删除:', removed);
  });
});
...
function createWindow() {
  const mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // 启用persistent storage
      webviewTag: true
    },
    icon: path.join(__dirname, 'icon.svg') // 使用SVG图标
  });

  // 加载网页
  mainWindow.loadURL('https://li.nstar.fun:8443');
...
});
{
  "name": "影视库",
  "short_name": "影视库",
  "description": "影视库PWA应用",
  "start_url": "https://li.nstar.fun:8443/",
  "display": "standalone",
  ...
  "icons": [
    {
      "src": "https://li.nstar.fun:8443/favicon.ico",
  ...
  "scope": "https://li.nstar.fun:8443/",
  "categories": ["entertainment", "video"]
}
{
  "name": "yingshiku",
  "version": "1.0.0",
  "description": "影视库Windows应用",
  "main": "main.js",
  "scripts": {
    "start": "electron .",
...
// preload.js
// 在渲染进程中执行，但可以访问Node.js API

// 这里可以添加一些预加载的功能，例如：
// 1. 注入自定义的JavaScript到网页
// 2. 设置IPC通信
// 3. 添加自定义的浏览器功能
...
这是一个使用 Electron 框架包装 https://li.nstar.fun:8443 的 Windows 桌面应用。
...
- `main.js` - Electron 主进程文件，负责创建浏览器窗口
- `preload.js` - 预加载脚本，用于主进程和渲染进程通信
- `index.html` - 备用页面（当网页加载失败时显示）
- `package.json` - 应用配置和依赖管理
- `icon.svg` - 应用图标（SVG格式）

## 图标说明

当前提供的是 SVG 格式的图标 `icon.svg`。在构建应用前，您需要将其转换为 ICO 格式并命名为 `icon.ico`，然后放置在项目根目录。

您可以使用在线工具或图像编辑软件进行转换，例如：
- [Convertio](https://convertio.co/svg-ico/)
- [CloudConvert](https://cloudconvert.com/svg-to-ico)

## 注意事项

1. 确保网络连接正常，以便应用能够访问 https://li.nstar.fun:8443
2. 首次安装依赖可能需要较长时间，请耐心等待
3. 构建应用需要 Node.js 环境（推荐使用 LTS 版本）
4. 应用的功能取决于原始网站的功能，本应用仅提供桌面访问方式
...
// 缓存名称和版本
const CACHE_NAME = 'yingshiku-pwa-v1';
const ASSETS_TO_CACHE = [
  '/',
  '/index.html',
  '/manifest.json',
  '/icon.svg',
  'https://li.nstar.fun:8443/'
];

// 安装Service Worker，缓存基本资源
...
self.addEventListener('fetch', (event) => {
  // 如果是远程API请求，直接转发
  if (event.request.url.startsWith('https://li.nstar.fun:8443')) {
    event.respondWith(
      fetch(event.request)
        .catch(() => {
          // 离线时返回自定义离线页面
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    clients.openWindow('https://li.nstar.fun:8443')
  );
});
